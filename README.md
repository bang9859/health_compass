...
### 🍈 이준호
 지난 한 달 동안 진행한 팀 프로젝트를 통해 초기 설계가 얼마나 중요한지 몸소 깨달았습니다. 처음으로 팀 단위로 서비스를 구축해보면서 완벽한 설계를 기대했지만, ERD나 테이블 설계 단계에서 몇 가지 중요한 요소들이 빠지거나 미흡한 부분이 드러나 처음부터 다시 수정해야 하는 상황이 잦았습니다. 게다가, 설계를 마친 후 개발을 시작하더라도 계획과는 다소 다른 방향으로 진행되는 경우가 있어, 개발과 수정 작업이 반복되면서 예상보다 훨씬 많은 시간이 소요되었습니다.
 이번 경험을 통해 초기 설계가 프로젝트 전체의 진행과 개발 기간에 큰 영향을 미친다는 점을 다시 한 번 느낄 수 있었습니다. 앞으로의 프로젝트에서는 초기 설계 단계에 더욱 신중을 기해, 오류나 개선할 부분이 없는지 꼼꼼히 검토할 필요가 있다고 생각합니다. 또한, 팀원들이 각자 맡은 기능들이 하나로 모여 문제없이 작동할 때의 성취감과 개발의 재미를 경험하면서, 팀워크와 협업의 중요성을 다시 한 번 실감하게 되었습니다.
이번 과정을 통해 팀원 간의 의견 공유와 활발한 피드백 문화가 프로젝트 성공에 얼마나 중요한 역할을 하는지 새삼 깨달았습니다. 앞으로의 프로젝트에서 팀워크를 강화하고 원활한 소통을 통해 성공적인 결과를 도출하는 데 좋은 경험이 된 프로젝트였습니다.
